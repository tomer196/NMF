...
class ParameterizedNMFSolver:
    ...
    def initialize_with_nmf(self, n_nmf_iterations=500):
        ...
        for iteration in range(n_nmf_iterations):
            ...
            total_loss, losses = self.loss_function(W_init, H_init, self.A_observed)
            ...
            if (iteration + 1) % 10000 == 0:
                print(f"  NMF iteration {iteration+1}/{n_nmf_iterations}, "
                      f"Total Loss: {total_loss.item():.6f}, "
                      f"Recon: {losses['fitting_loss'].item():.6f}, "
                      f"Penalty: {losses['penalty_loss'].item():.6f}, "
                      f"LR: {current_lr:.2e}")
        ...
        total_loss, losses = self.loss_function(W_param_matrix, H_param_matrix, self.A_observed)
        print(f"Parameterized approximation - Total: {total_loss.item():.6f} "
              f"(Recon: {losses['fitting_loss'].item():.6f}, Penalty: {losses['penalty_loss'].item():.6f})\n")
    ...
    def solve(self, n_iterations=1000, lr=0.01, print_every=10, use_scheduler=True, 
        ...
        for iteration in range(n_iterations):
            ...
            # Compute loss
            total_loss, losses = self.loss_function(self.W_param.matrix(), self.H_param.matrix(), self.A_observed)
